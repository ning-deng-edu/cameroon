			...
			if (newRecord) {
				...
				if(fileIdChange){
					...
					for(file : files_in_current_ques){
    					...
    					attributes.add(createEntityAttribute("FileID", null, null, tempAnsID+"_"+fileSuffix, null));
...
//For saving changed answer, we have five cases:
//1.Nothing changed, inform the user nothing have changed
//2.only file list changed, we just need to update file list info
...
saveChangedAnswer(){
		...
		//problem: wrong replacing field, using underscore"_"
			...
			String newAnsLabel=null;
				...
				String[] currentAnsLabelSplit=currentAnsLabel.split("_");
				...
				if (!tempAnsLabelInve.equals(currentAnsLabelInve)) {
					...
					if (ansLabelChange) {
						...
						onSave(uuid, newRecord) {
							...
							attributes = createAttributeList();
    						attributes.add(createEntityAttribute("FileID", null, null, newAnsLabel+"_"+fileSuffix, null));
							 saveArchEnt(entityId, "File", null, attributes, new SaveCallback() {
            					onSave(uuid,newRecord) {
            					//showWarning(entityId,tempAnsID+"-"+fileSuffix);
            					}  
		...
		case "answer":
			...
			switch (fileCategory){
				...
				case "Video":
					...
					setFieldValue("videoFile/videoFileInfo/videoFileID",tempAnsID+"_Video");
...
saveNewPerson(){
	...
	String personLabel=getFieldValue("person/personInfo/personName")+"_"+getFieldValue("person/personInfo/personDOB");
	//showWarning("personLabel",personLabel);
	...
	if(timeValidation(personDOB)){
		//showWarning("timevalid","timevalid");
	...
	fetchAll(loadAllAnswerQuery, new FetchCallback() {
        onFetch(result) {
            ...
            if(isNull(current_position)){
            	sssLabelTemp=username+"_uncertainPlace_"+currentDate;
            	sessionLabel="_uncertainPlace_"+currentDate;
        	}
            else{
            	sssLabelTemp=username+"_"+current_position.get(1)+"_"+current_position.get(2)+"_"+currentDate;
            	sessionLabel="_"+current_position.get(1)+"_"+current_position.get(2)+"_"+currentDate;
            }
            populateList("session/sessionFiles/sessionFileSelectionList", candidate_files_session);
            populateList("session/sessionFiles/sessionFileList", selected_files_session);
...
saveSession(String typeflag){
	...
	if(isNull(session_id)){//create new session
		...
		if(sessionInfoChange.containsKey("EQUAL")){
			if(sessionFileChange.containsKey("EQUAL")){
				...
				String [] cuSsLabelSplit=currentSessionLabel.split("\\s*[_]\\s*");
	...
	tempRmvItvOrigin.clear();
		...
		//String [] oldLabelItv=sssLabelOld.split("\\s*[-][(]\\s*");
			String [] oldLabelItv=sssLabelOld.split("\\s*[_]\\s*");
