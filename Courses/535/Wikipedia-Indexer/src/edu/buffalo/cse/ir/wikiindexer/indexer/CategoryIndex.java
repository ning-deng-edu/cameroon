...
public class CategoryIndex extends Index implements Writeable {

	private Properties props;
	private static CategoryIndex singletonInstance;
	

	private CategoryIndex(Properties props){
		this.props=props;		
		this.localDictionary=new LocalDictionary(props,INDEXFIELD.CATEGORY);
		this.indexEntries=new HashMap<Integer,LinkedList<Integer[]>>();
	}

	public static CategoryIndex getInstance(Properties props){
		if(singletonInstance==null){
			singletonInstance=new CategoryIndex(props);
	...
	public void writeToDisk() throws IndexerException {
		...
		try{			
			...
			File file = new File("categoryIndexFile.dat"); 
			if (!file.exists()) {
				file.createNewFile();
			}
	...
	public void readFromDisk(SharedDictionary sharedDict) throws IndexerException{
		try{ 
			...
			LinkedList<Integer[]> postingList;
			Integer[] docAndOccurranceMapping;
			RandomAccessFile randomAccessFile = new RandomAccessFile("categoryIndexFile.dat", "r");
